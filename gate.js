...
var server = http.createServer((req, res)=>{
    ...
    this.clientDistributor = new tcpClient("127.0.0.1"
    , 9000
    , (options)=>{
...
function onDistribute(data){
    for(var n in data.params){
        ...
        if(mapClients[key] === null && node.name != "gate"){
            var client = new tcpClient(node.host, node.port, onCreateClinet,onReadClinet, onEndClient, onErrorClient);
            mapClients[key] = {
                client:client,
                info:node
            };
...
function onCreateClinet(options){
    console.log("onCreateClient");
}

function onReadClinet(options, packet){
